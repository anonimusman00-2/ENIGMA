...
# Valid characters: uppercase/lowercase letters and numbers  
...
def get_message_input():  
    """  
    Function to collect message input with 5 consecutive Enter presses  
    All previous Enter presses will be included in the message  
    """  
    ...
    while empty_count < 5:  
        ...
        if line.strip() == "":  
            ...
            # If there's new text, add all previous Enter presses to the message  
    ...
    # Add the last 4 Enter presses to the message (if any)  
        ...
        if choice == '4':  
            ...
            if save == 'y':  
                try:  
                    ...
                    print(f"\nSuccessfully saved to file: {filename}")  
                except Exception as e:  
                    print(f"\nFailed to save to file: {e}")  
            
        elif choice in ['1', '2']:  
            try:  
                print("\nEnter rotor positions (any numbers) separated by spaces:")  
                ...
                print("(leave empty if you don't want to use plugboard)")  
                plugboard_pairs = parse_pairs(input("Plugboard: ").strip())  
                
                print("\nEnter Reflector pairs (example: Xy 9z 2N):")  
                print("(leave empty for default reflector)")  
                ...
                save_choice = input("\nDo you want to save the results to file? (y/n): ").lower().strip()  
                ...
                if save_choice == 'y':  
                    ...
                    if save_to_file(filename, message, result, operation, positions, plugboard_pairs, reflector_pairs):  
                        print(f"\n{operation.capitalize()} results have been saved to file: {filename}")  
